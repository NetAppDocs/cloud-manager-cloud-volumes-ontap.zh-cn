...
对实例的访问:: 从 Cloud Manager 创建 Cloud Volumes ONTAP HA 对时，系统会提示您为调解器实例提供一个密钥对。您可以使用 `admin` user 使用该密钥对进行 SSH 访问。
...
有关详细信息，请参见 link:reference-networking-aws.html#aws-networking-requirements-for-cloud-volumes-ontap-ha-in-multiple-azs["适用于多个 AWS 中的 Cloud Volumes ONTAP HA 的 AWS 网络要求"]。
