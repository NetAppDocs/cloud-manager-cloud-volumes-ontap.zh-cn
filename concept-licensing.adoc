...
* link:task-deploying-otc-aws.html["在 AWS 中启动 Cloud Volumes ONTAP"]
...
* link:task-deploying-otc-azure.html["在 Azure 中启动 Cloud Volumes ONTAP"]
...
* link:task-deploying-gcp.html["在 Google Cloud 中启动 Cloud Volumes ONTAP"]
...
* 在AWS中、可以从AWS市场的公有 产品中获得年度合同。部署Cloud Volumes ONTAP 工作环境时、Cloud Manager会提示您从"详细信息和凭据"页面订阅。
...
. link:task-manage-capacity-licenses.html["将许可证添加到 Cloud Manager"]
