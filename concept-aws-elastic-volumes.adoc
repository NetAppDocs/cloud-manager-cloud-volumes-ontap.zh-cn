...
* 目前、只有API才支持在初始聚合上启用弹性卷的情况下创建新的Cloud Volumes ONTAP 系统。不支持从Cloud Manager用户界面执行此操作。如果您从用户界面创建新的Cloud Volumes ONTAP 9.11.0或更高版本系统、则初始聚合不会启用弹性卷。
+
支持从用户界面执行所有其他操作(例如创建新聚合)。
...
* 创建一个在初始聚合上启用了弹性卷的新系统
+
如中所述 <<Limitations>>、目前只有API才支持此操作。用户界面不支持此功能。

+
启动API调用时、初始聚合默认启用弹性卷功能。

+
https://docs.netapp.com/us-en/cloud-manager-automation/cm/overview.html["了解如何使用API创建Cloud Volumes ONTAP"^]
