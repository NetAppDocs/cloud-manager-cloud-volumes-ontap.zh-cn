...
此页面上介绍的增强功能专用于支持 Cloud Volumes ONTAP 管理的 Cloud Manager 功能。一组单独的发行说明介绍了 Cloud Volumes ONTAP 软件本身的新增功能。
...
link:task-create-volumes.html["了解如何在特定聚合上创建卷"]。
...
link:task-manage-node-licenses.html#exchange-unassigned-node-based-licenses["了解如何交换未分配的基于节点的许可证"]。
